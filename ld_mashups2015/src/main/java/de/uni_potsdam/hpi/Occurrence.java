...
public class Occurrence {
    ...
    public Response getCoordinates(@QueryParam("longitude") double longitude, @QueryParam("latitude") double latitude,
        ...
        //FreebaseService fbService = new FreebaseService();
        dbPedia.includeDataFromDBpedia(occurrences.get(0).getSpecies());
        //fbService.includeDataFromFreebase(occurrences.get(0).getSpecies());
        WikimediaService wmService = new WikimediaService();
        wmService.includeDataFromWikimedia(occurrences.get(0).getSpecies());
    ...
    private JSONObject getJSONObjectFor(OccurrenceData occurrence) {
        ...
        if (null == occurrence.getSpecies().getThumbnailURL()) {
            if (!occurrence.getSpecies().getImageUrls().isEmpty()) {
                occurrenceJSON.put("thumbnailURL", occurrence.getSpecies().getImageUrls().get(0));
            }
        } else {
            occurrenceJSON.put("thumbnailURL", occurrence.getSpecies().getThumbnailURL());
        }
        occurrenceJSON.put("binomial", occurrence.getSpecies().getBinomial());
        occurrenceJSON.put("scientificName", occurrence.getSpecies().getScientificName() );
